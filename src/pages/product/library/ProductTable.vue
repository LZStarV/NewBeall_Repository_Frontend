<template>
  ...
  <lay-card>
    ...
    <div class="fixed-table-toolbar">
      <span data-title="AI填充数据" class="btnIcon invite-but" @click="handleAiFill">
        <lay-icon type="layui-icon-set" />
      </span>
      <span data-title="复制" class="btnIcon invite-but" @click="handleCopy">
        <lay-icon type="layui-icon-template" />
      </span>
      <span data-title="询价" class="btnIcon invite-but" @click="handleInquiry">
        <lay-icon type="layui-icon-dollar" />
      </span>
      <span data-title="显示重复产品" class="btnIcon invite-but" @click="handleShowDuplicate">
        <lay-icon type="layui-icon-table" />
      </span>
      <span data-title="单个新增" class="btnIcon invite-but" @click="handleSingleAdd">
        <lay-icon type="layui-icon-addition" />
      </span>
      <span data-title="批量新增" class="btnIcon invite-but" @click="handleBatchAdd">
      ...
      <span data-title="批量导出" class="btnIcon invite-but" @click="handleBatchExport">
        <lay-icon type="layui-icon-export" />
      </span>
      <span data-title="全库备份" class="btnIcon invite-but" @click="handleFullBackup">
        <lay-icon type="layui-icon-download-circle" />
      </span>
      <span data-title="品牌管理" class="btnIcon invite-but" @click="handleBrandManagement">
        <lay-icon type="layui-icon-diamond" />
      </span>
      <span data-title="添加至闲置云" class="btnIcon invite-but" @click="handleAddToIdleCloud">
...
type="button" aria-label="columns" class="btn btn-default btn-outline dropdown-toggle"
        ...
        <ul class="dropdown-menu" :class="{ 'show': showColumnsDropdown }">
          <li role="menuitem">
            <label>
              <input v-model="columnVisibility.proId" type="checkbox" @change="updateVisibleColumns" /> 产品编号
            </label>
          </li>
          <li role="menuitem">
            <label>
              <input v-model="columnVisibility.name" type="checkbox" @change="updateVisibleColumns" /> 产品名称
            </label>
          </li>
          <li role="menuitem">
            <label>
              <input v-model="columnVisibility.brand" type="checkbox" @change="updateVisibleColumns" /> 品牌
            </label>
          </li>
          <li role="menuitem">
            <label>
              <input v-model="columnVisibility.model" type="checkbox" @change="updateVisibleColumns" /> 型号
            </label>
          </li>
          <li role="menuitem">
            <label>
              <input v-model="columnVisibility.trait" type="checkbox" @change="updateVisibleColumns" /> 参数/特性
            </label>
          </li>
          <li role="menuitem">
            <label>
              <input v-model="columnVisibility.unit" type="checkbox" @change="updateVisibleColumns" /> 单位
            </label>
          </li>
          <li role="menuitem">
            <label>
              <input v-model="columnVisibility.purchaseprice" type="checkbox" @change="updateVisibleColumns" /> 成本
            </label>
          </li>
          <li role="menuitem">
            <label>
              <input v-model="columnVisibility.price" type="checkbox" @change="updateVisibleColumns" /> 参考售价
            </label>
          </li>
          <li role="menuitem">
            <label>
              <input v-model="columnVisibility.marketprice" type="checkbox" @change="updateVisibleColumns" /> 市场指导价
            </label>
          </li>
          <li role="menuitem">
            <label>
              <input v-model="columnVisibility.uname" type="checkbox" @change="updateVisibleColumns" /> 创建人
            </label>
          </li>
          <li role="menuitem">
            <label>
              <input v-model="columnVisibility.cloudLibrary" type="checkbox" @change="updateVisibleColumns" /> 云端库
            </label>
          </li>
          <li role="menuitem">
            <label>
              <input v-model="columnVisibility.openUpdate" type="checkbox" @change="updateVisibleColumns" /> 开放星标
            </label>
          </li>
          <li role="menuitem">
            <label>
              <input v-model="columnVisibility.designatedOpen" type="checkbox" @change="updateVisibleColumns" /> 指定开放
            </label>
          </li>
          <li role="menuitem">
            <label>
              <input v-model="columnVisibility.defaultDiscount" type="checkbox" @change="updateVisibleColumns" />
              默认折率(%)
            </label>
          </li>
          <li role="menuitem">
            <label>
              <input v-model="columnVisibility.autoHandle" type="checkbox" @change="updateVisibleColumns" /> 自动报价
            </label>
          </li>
        </ul>
      </div>
    ...
    <div class="content-area">
      <lay-table
:columns="visibleColumns" :data-source="tableData" :page="pagination" :loading="loading"
        ...
        <template #cloudLibrary="{ row }">
          <lay-switch
:model-value="row.isOpen === '1' || row.isOpen === 1"
            onswitch-text="ON" offswitch-text="OFF"
            :style="{
              '--lay-switch-on-color': '#5FB878',
              '--lay-switch-off-color': '#d9d9d9'
            }" @update:model-value="(value) => updateCloudLibraryStatus(row, value)" />
        </template>

        <!-- 开放星标列 -->
        <template #openUpdate="{ row }">
          <span class="action-icon" title="点击设置" @click="handleOpenPermissionManagement(row)">
        ...
        <template #designatedOpen="{ row }">
          <span class="action-icon" title="点击设置" @click="handleOpenDesignatedOpenManagement(row)">
            <SvgIcon name="set_up" width="20" height="20" />
          </span>
        </template>
  ...
  <!-- 询价弹窗 -->
  <InquiryModal
:visible="inquiryModalVisible" :selected-products="selectedProductsForInquiry"
